// ==============================================================
// File generated on Fri Mar 20 13:18:56 CDT 2020
...
//------------------------Parameter----------------------
`define TV_IN_a "/home/ares/FPGA/AXI_master_sim/test/tb_sample/c.example.autotvin_a.dat"
`define TV_OUT_a "/home/ares/FPGA/AXI_master_sim/test/tb_sample/rtl.example.autotvout_a.dat"
...
reg RLAST_tmp = 0;
reg RVALID_tmp = 0;
reg [a_DATA_BITWIDTH - 1 : 0] a_mem_0 [0: a_mem_depth - 1]; 
reg [a_DATA_BITWIDTH - 1 : 0] a_mem_1 [0: a_mem_depth - 1]; 
reg [a_DATA_BITWIDTH - 1 : 0] a_mem_2 [0: a_mem_depth - 1]; 
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > a_mem_depth) begin
                $display ("/home/ares/FPGA/HLS/axi_master/proj_axi_master/solution1/sim/verilog/AESL_axi_master_a.v: Write request address %d exceed AXI master a array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, a_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > a_mem_depth) begin
                $display ("/home/ares/FPGA/HLS/axi_master/proj_axi_master/solution1/sim/verilog/AESL_axi_master_a.v: Read request address %d exceed AXI master a array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, a_mem_depth); 
                $finish;
            end
...
initial begin : write_file_proc 
  ...
  while(1) begin 
      ...
      if (factor == 2) begin
          if ((a_mem_depth - 1) % factor == 0) begin
              case(mem_page)
                  0 : $fdisplay(fp,"0x%x",a_mem_0[a_mem_depth/factor][15:0]);
                  1 : $fdisplay(fp,"0x%x",a_mem_1[a_mem_depth/factor][15:0]);
                  2 : $fdisplay(fp,"0x%x",a_mem_2[a_mem_depth/factor][15:0]);
                  default: $display("The page_num of write file is not valid!");
              endcase
          end
      end
